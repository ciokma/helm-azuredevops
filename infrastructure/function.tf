...
# 6. Subir el zip como Blob privado
resource "azurerm_storage_blob" "function_code" {
  name                   = "my-function_code-content.zip"
  ...
  type                   = "Block"
  source                 = data.archive_file.python_function_package.output_path
}
...
resource "azurerm_function_app_flex_consumption" "function" {
  ...
  app_settings = {
    # Eliminar 'WEBSITE_RUN_FROM_PACKAGE' ya que el despliegue se hace aparte
    # Not supported
    ...
    # USER_ASSIGNED_IDENTITY_CLIENT_ID = azurerm_user_assigned_identity.function_identity.client_id
...
# 9. Despliegue de la Function App con un null_resource
...
#     # Use este si el comando az falla.
#     # command = "pwsh -c \"az functionapp deployment source config-zip -g ${azurerm_resource_group.rg.name} -n ${azurerm_function_app_flex_consumption.function.name} --src ${data.archive_file.python_function_package.output_path}\""
#     interpreter = ["/bin/bash", "-c"] # Use "cmd.exe" for Windows
#   }
# }
resource "null_resource" "function_deploy" {
  # Asegura que el recurso de la Function App se haya creado antes de intentar el despliegue
